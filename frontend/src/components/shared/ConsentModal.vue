...
const rules = () => {
  const raw = t('findpepper.consentRules') as unknown
...
<template>
  ...
  >
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 max-w-md w-full p-6">
      <h2 class="text-lg font-bold text-aws-dark dark:text-gray-100 mb-2">📍 {{ t('findpepper.consentTitle') }}</h2>
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">{{ t('findpepper.consentBody') }}</p>
