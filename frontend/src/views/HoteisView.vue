  ...
  <div>
    ...
    <div class="mt-8 bg-green-50 border border-green-200 rounded-xl p-6">
      <h3 class="text-lg font-bold text-green-800 mb-4">
        {{ {pt: '💡 Dicas da Comunidade', en: '💡 Community Tips', es: '💡 Tips de la Comunidad'}[locale] }}
      </h3>
      <ul class="space-y-3 text-sm text-green-900">
        <li>✅ {{ {pt: 'Treasure Island e Flamingo: melhor custo-benefício perto do Venetian (recomendados pela comunidade)', en: 'Treasure Island & Flamingo: best value near Venetian (community recommended)', es: 'Treasure Island y Flamingo: mejor costo-beneficio cerca del Venetian (recomendados por la comunidad)'}[locale] }}</li>
        <li>✅ {{ {pt: 'Excalibur e OYO: baratos e ao lado do MGM Grand (monorail gratuito até o Venetian em 5 min)', en: 'Excalibur & OYO: cheap and next to MGM Grand (free monorail to Venetian in 5 min)', es: 'Excalibur y OYO: baratos y al lado del MGM Grand (monorail gratuito hasta el Venetian en 5 min)'}[locale] }}</li>
        <li>✅ {{ {pt: 'Rachar quarto: 2 pessoas tranquilo (2 camas). 3-4 pessoas paga taxa extra (~$35-50). Reserve pelo Booking para escolher tipo de quarto.', en: 'Share room: 2 people easy (2 beds). 3-4 people pay extra fee (~$35-50). Book via Booking to choose room type.', es: 'Compartir habitación: 2 personas tranquilo (2 camas). 3-4 personas pagan tarifa extra (~$35-50). Reserva por Booking para elegir tipo de habitación.'}[locale] }}</li>
        <li>✅ {{ {pt: 'CVC pode ter preços melhores que Booking + parcela em 10x + suporte PT. Mas confirme se resort fee está inclusa!', en: 'CVC (Brazilian agency) may have better prices + 10x installments + PT support. But confirm if resort fee is included!', es: 'CVC (agencia brasileña) puede tener mejores precios + cuotas en 10x + soporte PT. ¡Pero confirma si el resort fee está incluido!'}[locale] }}</li>
        <li>⚠️ {{ {pt: 'Atenção: resort fee ($30-50/dia) geralmente NÃO está no preço anunciado. Calcule o custo total antes de reservar!', en: 'Warning: resort fee ($30-50/day) usually NOT included in advertised price. Calculate total cost before booking!', es: '¡Atención: resort fee ($30-50/día) generalmente NO está en el precio anunciado. ¡Calcula el costo total antes de reservar!'}[locale] }}</li>
      </ul>
    </div>
  </div>
</template>
