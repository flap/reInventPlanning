...
const cronograma = computed(() => {
  const data = {
    pt: [
      ...
      { dia: 'Domingo', data: '29 Nov', atividades: 'Badge pickup, workshops pré-evento, registro' },
      { dia: 'Segunda', data: '30 Nov', atividades: 'Keynote de abertura, sessões, Expo Hall abre' },
      { dia: 'Terça', data: '1 Dez', atividades: 'Keynote, sessões técnicas, builders sessions' },
      { dia: 'Quarta', data: '2 Dez', atividades: 'Keynote, sessões, workshops, networking' },
      { dia: 'Quinta', data: '3 Dez', atividades: 're:Inforce (segurança dedicado), sessões avançadas' },
      { dia: 'Sexta', data: '4 Dez', atividades: 'Keynote CTO, últimas sessões, re:Play party, encerramento' },
    ],
    en: [
      { dia: 'Saturday', data: 'Nov 28', atividades: 'Arrival, explore the Strip, adjust to timezone' },
      { dia: 'Sunday', data: 'Nov 29', atividades: 'Badge pickup, pre-event workshops, registration' },
      { dia: 'Monday', data: 'Nov 30', atividades: 'Opening Keynote, sessions, Expo Hall opens' },
      { dia: 'Tuesday', data: 'Dec 1', atividades: 'Keynote, technical sessions, builders sessions' },
      { dia: 'Wednesday', data: 'Dec 2', atividades: 'Keynote, sessions, workshops, networking' },
      { dia: 'Thursday', data: 'Dec 3', atividades: 're:Inforce (dedicated security day), advanced sessions' },
      { dia: 'Friday', data: 'Dec 4', atividades: 'CTO Keynote, final sessions, re:Play party, closing' },
    ],
    es: [
      { dia: 'Sábado', data: '28 Nov', atividades: 'Llegada, explorar la Strip, ajustar al huso horario' },
      { dia: 'Domingo', data: '29 Nov', atividades: 'Recogida de badge, talleres previos, registro' },
      { dia: 'Lunes', data: '30 Nov', atividades: 'Keynote de apertura, sesiones, Expo Hall abre' },
      { dia: 'Martes', data: '1 Dic', atividades: 'Keynote, sesiones técnicas, builders sessions' },
      { dia: 'Miércoles', data: '2 Dic', atividades: 'Keynote, sesiones, talleres, networking' },
      { dia: 'Jueves', data: '3 Dic', atividades: 're:Inforce (seguridad dedicado), sesiones avanzadas' },
      { dia: 'Viernes', data: '4 Dic', atividades: 'Keynote CTO, últimas sesiones, re:Play party, cierre' },
    ],
  }
  return data[locale.value] || data.en
})
    ...
    <section class="mb-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          ...
          :key="session.code"
          class="bg-white border border-gray-200 rounded-xl p-4 hover:border-aws-orange/50 transition-colors"
        >
          <span class="inline-block bg-aws-dark text-white text-xs font-mono font-bold px-2 py-0.5 rounded mb-2">
            {{ session.code }}
          </span>
          <h3 class="font-semibold text-aws-dark text-sm">{{ session.nome }}</h3>
          <p class="text-xs text-gray-500 mt-1">{{ session.descricao }}</p>
        </div>
      </div>
    </section>
    ...
    <section class="mb-10">
      ...
      <div class="flex flex-wrap gap-3">
        <span
          v-for="n in niveis"
          :key="n.nivel"
          :class="[n.cor, 'px-4 py-2 rounded-full text-sm font-semibold']"
        >
          {{ n.nivel }} — {{ n.label }}
        </span>
      </div>
    ...
    <section class="mb-10">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm border border-gray-200 rounded-xl overflow-hidden">
          ...
          <tbody>
            <tr
              v-for="(item, i) in cronograma"
              :key="item.dia"
              :class="i % 2 === 0 ? 'bg-white' : 'bg-gray-50'"
            >
              <td class="px-4 py-3 font-medium text-aws-dark">{{ item.dia }}</td>
              <td class="px-4 py-3 text-gray-500">{{ item.data }}</td>
              <td class="px-4 py-3 text-gray-700">{{ item.atividades }}</td>
            </tr>
          </tbody>
        </table>
      </div>
